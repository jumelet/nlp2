    ...
    with open(alignment_path) as f:
        ...
        for l in lines:
            ...
            if l[3] == 'S':
                ...
                align_p[int(l[0])].append((int(l[1]), int(l[2])))  # S also in P!
            else:
...
def calc_multiple_accuracy(en, fr, align_p):
    ...
    def get_alignments(s, t):
        ...
        for i, wt in enumerate(t, start=1):
            ...
            if maxlink != 0 and ((s[maxlink] in s[maxlink+1:] or s[maxlink] in s[:maxlink])): # and maxlink not in maxlinks):
...
if __name__ == '__main__':
    ...
    en_pos, en_sen = read_pos('../data/tagged/dev.pos.e', '../data/testing/test/test.e')
    fr_pos, fr_sen = read_pos('../data/tagged/dev.pos.f', '../data/testing/test/test.f')
    ...
    if experiments['pos_alignments']:
        ...
        pprint(sure_pos_diff.most_common(mc))

        print('\nmost common pos2pos in probable-sure links:')
        pprint(prob_pos_diff.most_common(mc))

        print('\nmost common wrong pos2pos links')
        pprint(wrong_pos_diff.most_common(mc))
